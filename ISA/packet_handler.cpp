...
void packet_handler(u_char *user, const struct pcap_pkthdr * header, const u_char *packet){
    ...
    if(IPvX == 0x40 ){
        ...
        unsigned dataOffset = ( *(packet + IPvX_OFFSET) & 0x0F) * 4;
        // ip addreses from IPv4 header packet
        uint32_t srcIp = *(packet + (IPvX_OFFSET + 12) );
        uint32_t dstIp = *(packet + (IPvX_OFFSET + 16) );

        if( Protocol == 6 /*TCP*/){
            uint32_t srcPort = *(packet + (IPvX_OFFSET + dataOffset) );
            uint32_t dstPort = *(packet + (IPvX_OFFSET + dataOffset + 2) );
            ...
            record = std::make_shared<Record>(srcIp, dstIp, 0, 0, "icmp", header->len, true );
        }
        else return;
    }
    // IPv6
        ...
        __uint128_t srcIp = *(packet + (IPvX_OFFSET + 8));
        __uint128_t dstIp = *(packet + (IPvX_OFFSET + 24));

        if( Protocol == 6 /*TCP*/){
            uint32_t srcPort = *(packet + (IPv6_DATA_OFFSET));
            uint32_t dstPort = *(packet + (IPv6_DATA_OFFSET + 2));
